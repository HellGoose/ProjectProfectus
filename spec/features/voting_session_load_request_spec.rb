...
feature "Users parallell voting requests" do
	before do
		...
		for j in 1..10
			t = Thread.new do
				...
				puts 3
				#get '/'
				visit '/vote/campaign/-1', format: :js # 'Make a difference'
				puts 4
				for i in 0..3
					#page.find(:xpath, '//*[@id='+Random.rand(0..2).to_s+']/a').click
					visit '/vote/campaign/'+Random.rand(0..2).to_s
				end
				raise "OK! "+j.to_s
				@n+=1
				Process.waitall
	...
	scenario "Should render website properly for user X", :js => true do 
		sleep 10
		if @n == 10
			visit '/'
			puts "finished!"
