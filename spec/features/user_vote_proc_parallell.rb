=begin
This does not work, because the test somehow finishes
before the forks finish, which causes an after_hook
error, as capybara does not like forks.
=end
...
feature "Campaign voting" do
	...
	scenario "by going through voting process", :js => true do
		...
		#$users = 5.times.map do
			#f = fork do
			#puts n
			forks[n]=Thread.new(n) {
				#puts "thread: "+n.to_s
				begin 
					voter = create(:user, name:"Kari"+n.to_s, uid: 10000+n)
					...
					#save_screenshot('screenshots/cap'+n.to_s+'.png')
					puts voter.name+" voted!"
					current_window.close
					success = success + 1
				rescue TimeoutError
					puts "Failed to connect! Timed out! "
				rescue Exception => e
					puts "Fail. Something strange has happened..... Error name is #{e.class}"
					puts e.message
				end
				puts $HORE
				$HORE += 1
				if $HORE == 0
					visit root_path
					puts "Finished!\n" + success.to_s + " sucesses \n" + ($HORE - success).to_s + "failures"
				end
				exit!
			#end
			#forks.push(f)
		...
		#puts forks
		forks.each { |fork| Process.waitpid(fork, 0)}
		#Process.waitpid(f, 0)
		#sleep 40
	end
end